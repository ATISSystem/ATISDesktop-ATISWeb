...
namespace ATISMobileRestful.Controllers
{
    ...
    {
        [HttpGet]
        public Models.TruckDriver GetTruckDriver(Int64 YourMUId)
        {
            var Item = new Models.TruckDriver();
            try
            {
                var TruckDriver = R2CoreTransportationAndLoadNotificationMClassTruckDriversManagement.GetNSSTruckDriver(R2CoreTransportationAndLoadNotificationMClassMobileUsersManagement.GetNSSMobileUser(YourMUId));
