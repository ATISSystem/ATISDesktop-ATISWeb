...
Namespace CarTruckNobatManagement
    ...
    Public Class PayanehClassLibraryMClassCarTruckNobatManager
        ...
        Public Sub TurnsCancellation(YourTopSequentialTurnNumber As String, YourNSSSequentialTurn As R2CoreTransportationAndLoadNotificationStandardSequentialTurnStructure, YourYearShamsi As String, NSSSoftwareUser As R2CoreStandardSoftwareUserStructure)
            ...
            Try
                ...
                InstanceSQLInjectionPrevention.GeneralAuthorization(YourTopSequentialTurnNumber)
                InstanceSQLInjectionPrevention.GeneralAuthorization(YourYearShamsi)

                Dim InstancePermissions = New R2CoreInstansePermissionsManager
                If Not InstancePermissions.ExistPermission(R2CoreTransportationAndLoadNotificationPermissionTypes.SoftwareUserCanExcecuteTurnCancellation, NSSSoftwareUser.UserId, 0) Then Throw New PermissionException

                If Split(YourTopSequentialTurnNumber, ":")(0).Length < 6 Then
                    Dim InstancePublicProcedures = New R2CoreInstancePublicProceduresManager
                    YourTopSequentialTurnNumber = InstancePublicProcedures.RepeatStr("0", 6 - Split(YourTopSequentialTurnNumber, ":")(0).Length) + YourTopSequentialTurnNumber
                End If
                ...
                Da.SelectCommand = New SqlClient.SqlCommand("
                       Select nEnterExitId from dbtransport.dbo.TbEnterExit as Turns
                        ...
                        Where (SUBSTRING(Turns.OtaghdarTurnNumber,2,20) <  '" & YourYearShamsi + "/" + YourTopSequentialTurnNumber & "') and 
                              ...
                              SUBSTRING(Turns.OtaghdarTurnNumber,1,1)='" & YourNSSSequentialTurn.SequentialTurnKeyWord & "' 
                ...
                For Loopx As Int64 = 0 To Ds.Tables(0).Rows.Count - 1
                    Dim NSSTruck = InstanceTrucks.GetNSSTruck(InstanceTurns.GetNSSTruck(Convert.ToInt64(Ds.Tables(0).Rows(Loopx).Item("nEnterExitId"))).NSSCar.nIdCar)
                    TWSClassTDBClientManagement.DelNobat(NSSTruck.NSSCar.StrCarNo, NSSTruck.NSSCar.StrCarSerialNo)
                Next
    ...
    Public Class PayanehClassLibraryMClassCarTruckNobatManagement
        ...
        Public Shared Function GetListOfAllnEnterExitId(YourNSSSequentialTurn As R2CoreTransportationAndLoadNotificationStandardSequentialTurnStructure) As List(Of String)
            Try
                Dim Ds As DataSet
                If R2ClassSqlDataBOXManagement.GetDataBOX(New R2ClassSqlConnectionSepas,
                        "Select LTRIM(RTRIM(Substring(Turns.OtaghdarTurnNumber,7,20))) as OtaghdarTurnNumber,Turns.StrEnterDate from dbtransport.dbo.TbEnterExit as Turns
                           Inner Join R2PrimaryTransportationAndLoadNotification.dbo.TblSequentialTurns as SequentialTurns On SUBSTRING(Turns.OtaghdarTurnNumber,1,1) Collate Arabic_CI_AI_WS = SequentialTurns.SeqTKeyWord  Collate Arabic_CI_AI_WS
                         Where bFlagDriver=0 and Substring(OtaghdarTurnNumber,1,1)='" & YourNSSSequentialTurn.SequentialTurnKeyWord & "' AND SequentialTurns.Active=1 and SequentialTurns.Deleted=0
                         Order By nEnterExitId Desc", 1, Ds).GetRecordsCount() = 0 Then
        ...
        Public Shared Function GetReserveTurn(YourTurnRegisterRequestId As Int64, YourUserNSS As R2CoreStandardSoftwareUserStructure) As Int64
        ...
        Public Shared Function GetTurnofKiosk(YourNSSTruck As R2CoreTransportationAndLoadNotificationStandardTruckStructure, YourNSSTruckDriver As R2CoreTransportationAndLoadNotificationStandardTruckDriverStructure, YourNSSTransportCompany As R2CoreTransportationAndLoadNotificationStandardTransportCompanyStructure, YourNSSLoadCapacitorLoad As R2CoreTransportationAndLoadNotificationStandardLoadCapacitorLoadStructure, YourUserNSS As R2CoreStandardSoftwareUserStructure) As Int64
            Dim CmdSql As SqlCommand = New SqlCommand
            ...
            Try
                ...
                CmdSql.CommandText = "Insert into dbtransport.dbo.TbCarAndPerson(nIDCar,nIDPerson,snRelation,dDate,strDesc,RelationTimeStamp)
                  				      Values(" & YourNSSTruck.NSSCar.nIdCar & "," & YourNSSTruckDriver.NSSDriver.nIdPerson & ",2,'" & _DateTime.GetCurrentDateShamsiFull & "','TempRelation','" & _DateTime.GetCurrentDateTimeMilladiFormated() & "')"
                CmdSql.ExecuteNonQuery()
                CmdSql.CommandText = "Insert Into R2PrimaryParkingSystem.dbo.TblTrafficCardsRelationCars(CardId, nCarId, RelationActive, RelationTimeStamp) 
                  				      Values(" & NSSMoneyWallet.CardId & "," & YourNSSTruck.NSSCar.nIdCar & ",1,'" & _DateTime.GetCurrentDateTimeMilladiFormated() & "')"
                CmdSql.ExecuteNonQuery()
                CmdSql.CommandText = "Insert Into R2PrimaryTransportationAndLoadNotification.dbo.TblAnnouncementHallSubGroupsRelationCars(CarId,AHSGId,Priority,RelationActive,RelationTimeStamp)
	         				          Values(" & YourNSSTruck.NSSCar.nIdCar & "," & YourNSSLoadCapacitorLoad.AHSGId & ",1,1,'" & _DateTime.GetCurrentDateTimeMilladiFormated() & "')"
                CmdSql.ExecuteNonQuery()
                CmdSql.Transaction.Commit() : CmdSql.Connection.Close()
        ...
        Public Shared Sub TurnsCancellation(YourTopSequentialTurnNumber As String, YourNSSSequentialTurn As R2CoreTransportationAndLoadNotificationStandardSequentialTurnStructure, YourYearShamsi As String)
            Dim CmdSql As New SqlClient.SqlCommand
            CmdSql.Connection = (New R2ClassSqlConnectionSepas).GetConnection()
            Try
                If Split(YourTopSequentialTurnNumber, ":")(0).Length < 6 Then
        ...
        Private Shared _TurnsCancellationBaseOnDurationProcessExcecutedFlag As Boolean = False
        Public Shared Sub TurnsCancellationBaseOnDuration()
            Dim CmdSql As New SqlClient.SqlCommand
            CmdSql.Connection = (New R2ClassSqlConnectionSepas).GetConnection()
            Try
                ...
                If TimeOfDay >= "00:00:00" And TimeOfDay <= "00:05:00" Then
                    _TurnsCancellationBaseOnDurationProcessExcecutedFlag = False
                    Return
                ElseIf TimeOfDay < InstanceConfigurations.GetConfigString(R2CoreTransportationAndLoadNotificationConfigurations.AnnouncementHallsTurnCancellationSetting, 1) Then
                    Return
                Else
                End If
                ...
                If _TurnsCancellationBaseOnDurationProcessExcecutedFlag Then Return
                ...
                For Loopx As Int64 = 0 To LstSeqTs.Count - 1
                    ...
                    Dim ConfigSeqTTurnsCancellationDuration As Int64 = Split(Mid(AllSeqTConfig.Where(Function(x) Mid(x, 1, ComposeSearchString.Length) = ComposeSearchString)(0), ComposeSearchString.Length + 1, AllSeqTConfig.Where(Function(x) Mid(x, 1, ComposeSearchString.Length) = ComposeSearchString)(0).Length), ":")(1)
                    ...
                    If TotalTurns > 0 Then
                        ...
                        CmdSql.CommandText = "Update dbtransport.dbo.TbEnterExit
                                              Set TurnStatus=" & TurnStatuses.CancelledUnderScore & ",bFlag=1,bFlagDriver=1 
                                              Where SUBSTRING(OtaghdarTurnNumber,1,1) = '" & LstSeqTs(Loopx).SequentialTurnKeyWord & "' and 
                                                    (TurnStatus=" & TurnStatuses.Registered & " or TurnStatus=" & TurnStatuses.UsedLoadAllocationRegistered & "  or TurnStatus=" & TurnStatuses.ResuscitationLoadAllocationCancelled & "  or TurnStatus=" & TurnStatuses.ResuscitationLoadPermissionCancelled & " or TurnStatus=" & TurnStatuses.ResuscitationUser & ") and 
                                                    StrEnterDate<='" & TopFirstDayToCancell & "'"
                        CmdSql.ExecuteNonQuery()
                        CmdSql.Connection.Close()
                        'در آنلاین
    ...
    Public Class TurnRegisteringStrategy
        ...
        Public Sub New(YourNSSTrafficCard As R2CoreParkingSystemStandardTrafficCardStructure, YourNSSTurnRegisteringRequest As R2CoreTransportationAndLoadNotificationStandardTurnRegisterRequestStructure, YourUserNSS As R2CoreStandardSoftwareUserStructure, YourRequesterId As Int64)
            Try
    ...
    Public Class RealTimeTurnRegisteringStrategy
        ...
        Public Sub New(YourNSSTrafficCard As R2CoreParkingSystemStandardTrafficCardStructure, YourNSSTurnRegisteringRequest As R2CoreTransportationAndLoadNotificationStandardTurnRegisterRequestStructure, YourUserNSS As R2CoreStandardSoftwareUserStructure, YourRequesterId As Int64)
            MyBase.New(YourNSSTrafficCard, YourNSSTurnRegisteringRequest, YourUserNSS, YourRequesterId)
        End Sub
    ...
    Public Class ReserveTurnRegisteringStrategy
        ...
        Private Sub DoStrategyRequestedEvent_Handler() Handles MyClass.DoStrategyRequestedEvent
            ...
            Try
                ...
                'احیاء نوبت رزرو
                CmdSql.CommandText = "Update dbtransport.dbo.tbEnterExit 
                                      Set strCardno=" & NSSTruck.NSSCar.nIdCar & ",strDriverName='" & NSSDriverTruck.NSSDriver.StrPersonFullName & "',strDesc=strDesc+' - " & NSSSoftwareUser.UserName & "',bFlag=0,bFlagDriver=0,nDriverCode=" & NSSDriverTruck.NSSDriver.nIdPerson & ",OtaghdarTurnNumber='" & SequentialTurnId_ & "',TurnStatus=" & TurnStatuses.Registered & " 
...
Namespace TurnRegisterRequest
    ...
    Public NotInheritable Class PayanehClassLibraryMClassTurnRegisterRequestManager
        ...
        Public Sub RealTimeTurnRegisterRequestWithLicensePlate(YourLPPelak As String, YourLPSerial As String, YourNSSSoftwareUser As R2CoreStandardSoftwareUserStructure, YourRequesterId As Int64)
            Try
                Dim InstanceSQLInjectionPrevention = New R2CoreSQLInjectionPreventionManager
                ...
                If InstanceCarTrucks.IsExistCarTruckWithLicensePlate(New R2StandardCarTruckStructure(NSSTruck.NSSCar, Nothing), TruckId) Then
                    ...
                    Dim TurnId As Int64
                    PayanehClassLibraryMClassTurnRegisterRequestManagement.RealTimeTurnRegisterRequest(NSSTruck, False, True, TurnId, YourRequesterId, YourNSSSoftwareUser)
                Else
    ...
    Public NotInheritable Class PayanehClassLibraryMClassTurnRegisterRequestManagement
        ...
        Public Shared Function RealTimeTurnRegisterRequest(YourNSSTruck As R2CoreTransportationAndLoadNotificationStandardTruckStructure, YourTurnPrintRedirect As Boolean, YourMoneyWalletInventoryControl As Boolean, ByRef YourTurnId As Int64, YourRequesterId As Int64, YourUserNSS As R2CoreStandardSoftwareUserStructure) As Int64
            Try
                Dim NSSTrafficCard = R2CoreParkingSystemMClassTrafficCardManagement.GetNSSTrafficCard(R2CoreParkingSystemMClassCars.GetCardIdFromnIdCar(YourNSSTruck.NSSCar.nIdCar))
                ...
                'ثبت نوبت
                Dim Strategy = New RealTimeTurnRegisteringStrategy(NSSTrafficCard, R2CoreTransportationAndLoadNotificationMClassTurnRegisterRequestManagement.GetNSSTurnRegisterRequest(TurnRegisterRequestId), YourUserNSS, YourRequesterId)
                Strategy.DoStrategy()
        ...
        Public Shared Sub EmergencyTurnRegister(YourDataStruct As DataStruct, YourTurnPrintRedirect As Boolean, YourRequesterId As Int64, YourUserNSS As R2CoreStandardSoftwareUserStructure)
            Try
                ...
                'ثبت نوبت
                Dim Strategy = New RealTimeTurnRegisteringStrategy(NSSTrafficCard, R2CoreTransportationAndLoadNotificationMClassTurnRegisterRequestManagement.GetNSSTurnRegisterRequest(YourDataStruct.Data2), YourUserNSS, YourRequesterId)
                Strategy.DoStrategy()
                Dim TurnId = Strategy.GetNSSTurn.nEnterExitId
                'ثبت رابطه نوبت با درخواست صدور نوبت از طریق فضانام مدیریت روابط نهادی
                R2CoreMClassEntityRelationManagement.RegisteringEntityRelation(New R2StandardEntityRelationStructure(Nothing, R2CoreTransportationAndLoadNotificationEntityRelationTypes.Turn_TurnRegisterRequest, TurnId, YourDataStruct.Data2, Nothing), RelationDeactiveTypes.BothDeactive)
                'درخواست چاپ نوبت
        ...
        Public Shared Function ReserveTurnRegisterRequest(YourComputerId As Int64, YourUserNSS As R2CoreStandardSoftwareUserStructure) As Int64
            Try
                'کنترل مجوز
                Dim InstancePermissions = New R2CoreInstansePermissionsManager
                ...
                'ثبت نوبت
                Dim TurnId = PayanehClassLibraryMClassCarTruckNobatManagement.GetReserveTurn(TurnRegisterRequestId, YourUserNSS)
                'ثبت رابطه نوبت با درخواست صدور نوبت از طریق فضانام مدیریت روابط نهادی
        ...
        Public Shared Function ResuscitationReserveTurn(YourTurnRegisterRequestId As Int64, YourNSSTruck As R2CoreTransportationAndLoadNotificationStandardTruckStructure, YourMoneyWalletInventoryControl As Boolean, YourRequesterId As Int64, YourUserNSS As R2CoreStandardSoftwareUserStructure) As Int64
            Try
                Dim InstancePermissions = New R2CoreInstansePermissionsManager
                ...
                If DateDiff(DateInterval.Hour, NSSTRR.DateTimeMilladi, _DateTime.GetCurrentDateTimeMilladi) > R2CoreTransportationAndLoadNotificationMClassTurnRegisterRequestManagement.GetNSSTurnRegisterRequestType(NSSTRR.TRRTypeId).TurnExpirationHours Then Throw New TurnRegisteringRequestDateTimeExpiredException
                Dim NSSTrafficCard = R2CoreParkingSystemMClassTrafficCardManagement.GetNSSTrafficCard(R2CoreParkingSystemMClassCars.GetCardIdFromnIdCar(YourNSSTruck.NSSCar.nIdCar))
                'کنترل موجودی کیف پول برای هزینه صدور نوبت - درصورتی که موجودی کافی نباشداکسپشن پرتاب می گردد 
                If YourMoneyWalletInventoryControl Then If Not IsMoneyWalletInventoryIsEnoughForTurnRegistering(NSSTrafficCard) Then Throw New MoneyWalletCurrentChargeNotEnoughException
                'ثبت نوبت
                Dim Strategy = New ReserveTurnRegisteringStrategy(NSSTrafficCard, R2CoreTransportationAndLoadNotificationMClassTurnRegisterRequestManagement.GetNSSTurnRegisterRequest(YourTurnRegisterRequestId), YourUserNSS, YourRequesterId)
                Strategy.DoStrategy()
